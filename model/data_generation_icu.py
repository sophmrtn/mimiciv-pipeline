...
class Generator:
    ...
    def generate_adm(self):
        ...
        data[['days','dummy','hours']] = data['los'].str.rsplit(' ', expand=True)
        data[['hours','min','sec']] = data['hours'].str.rsplit(':', expand=True)
    ...
    def generate_proc(self):
        ...
        proc[['start_days', 'dummy','start_hours']] = proc['event_time_from_admit'].str.rsplit(' ', expand=True)
        proc[['start_hours','min','sec']] = proc['start_hours'].str.rsplit(':', expand=True)
    ...
    def generate_out(self):
        ...
        out[['start_days', 'dummy','start_hours']] = out['event_time_from_admit'].str.rsplit(' ', expand=True)
        out[['start_hours','min','sec']] = out['start_hours'].str.rsplit(':', expand=True)
    ...
    def generate_chart(self):
        ...
        for chart in tqdm(pd.read_csv("./data/features/preproc_chart_icu.csv.gz", compression='gzip', header=0, index_col=None,chunksize=chunksize)):
            ...
            chart[['start_days', 'dummy','start_hours']] = chart['event_time_from_admit'].str.rsplit(' ', expand=True)
            chart[['start_hours','min','sec']] = chart['start_hours'].str.rsplit(':', expand=True)
    ...
    def generate_meds(self):
        ...
        meds[['start_days', 'dummy','start_hours']] = meds['start_hours_from_admit'].str.rsplit(' ', expand=True)
        meds[['start_hours','min','sec']] = meds['start_hours'].str.rsplit(':', expand=True)
        meds['start_time']=pd.to_numeric(meds['start_days'])*24+pd.to_numeric(meds['start_hours'])
        meds[['start_days', 'dummy','start_hours']] = meds['stop_hours_from_admit'].str.rsplit(' ', expand=True)
        meds[['start_hours','min','sec']] = meds['start_hours'].str.rsplit(':', expand=True)
